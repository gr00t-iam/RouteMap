...
export default function TechniciansPage() {
  ...
  const handleAdd = () => {
    ...
    addTechnician({ id: `tech-${Date.now()}`, name: trimmed, color });
    setName('');
  ...
  const handleSaveEdit = (id: string) => {
    ...
    if (!trimmed) return;
    setTechnicians(technicians.map((t) => t.id === id ? { ...t, name: trimmed, color: editColor } : t));
    setEditId(null);
  };
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto space-y-4">
      ...
      <p className="text-sm text-slate-500">
        Add technicians here, then go to <strong>Routes</strong> to assign stops and export each person's route sheet.
      </p>
      ...
      <div className="card p-4 space-y-3">
        ...
        <div className="flex gap-3 items-end flex-wrap">
          <div className="flex-1 min-w-[160px]">
            ...
            <input className="input" placeholder="e.g. John Smith" value={name}
          ...
          <button className="btn-primary" onClick={handleAdd} disabled={!name.trim()}>Add</button>
        </div>
      </div>
              ...
              {editId === t.id ? (
                <>
                  ...
                  <input className="input flex-1" value={editName} onChange={(e) => setEditName(e.target.value)}
                    onKeyDown={(e) => e.key === 'Enter' && handleSaveEdit(t.id)} autoFocus />
                ...
                <>
                  ...
                  <button className="btn-ghost text-xs px-2" onClick={() => startEdit(t)}>Edit</button>
                  <button className="btn-ghost text-xs px-2 text-rose-500 hover:bg-rose-50" onClick={() => handleRemove(t.id)}>Remove</button>
                </>
              )}
            </div>
          ))}
      ...
      {technicians.length > 0 && (
        <p className="text-xs text-slate-400 text-center">
          {technicians.length} technician{technicians.length !== 1 ? 's' : ''} · Go to <strong>Routes</strong> to assign stops and export route sheets
        </p>
      )}
    </div>
  );
